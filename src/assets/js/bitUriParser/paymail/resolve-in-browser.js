...
import paymail from "../../paymail-client/paymail-client.browserified";
...
export async function resolve(
  ...
  var client = new paymail.PaymailClient(null, null, null, bsv);
  ...
  senderInfo.signature = paymail.VerifiableMessage.forBasicAddressResolution(
    senderInfo
  ).sign(privkeyOfPaymailUser);

  var out = await client.getOutputFor(paymailAddress, senderInfo);
  return out;
}

export async function findPeerEndpoint(paymailString, o = defaultOptions) {
  o = { ...defaultOptions, ...o };
